    			...
    			foreach ( $this->fields as $field ) {
    				if ( isset( $field['type'] ) ){
    					if( 'color' === $field['type'] ){ 
                            ...
                            wp_enqueue_script( 'all', get_template_directory_uri() . '/js' . $unminify . '/all' . $suffix . '.js', array( 'jquery' ), '5.3.1', true );
                            wp_enqueue_script( 'v4-shims', get_template_directory_uri() . '/js' . $unminify . '/v4-shims' . $suffix . '.js', array( 'jquery' ), '5.3.1', true );
    					}
    				}
    			}
    		}
    							...
    							<# } else if ( 'color' === field.type ) { #>
    						        ...
    						        <label>
    						            ...
    						            <# if ( field.description ) { #>
    						                <span class="description customize-control-description">{{{ field.description }}}</span>
    						            <# } #>
    						            <input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php echo esc_attr__( 'Hex Value', 'spa-and-salon-pro' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
