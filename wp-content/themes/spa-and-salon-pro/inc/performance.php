...
if( ! function_exists( 'spa_and_salon_pro_image_lazy_load_attr' ) ) :
/**
 ...
 * @return array
 */
function spa_and_salon_pro_image_lazy_load_attr( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'spa_and_salon_pro_image_lazy_load_attr', 10, 3 );

if( ! function_exists( 'spa_and_salon_pro_content_image_lazy_load_attr' ) ) :
/**
 ...
 * @return string
 */
function spa_and_salon_pro_content_image_lazy_load_attr( $content ) {
	$ed_lazyload_content = get_theme_mod( 'ed_lazy_load_cimage', false );
	
    if ( $ed_lazyload_content && ! empty( $content ) ) {
		$content = preg_replace_callback(
			'/<img([^>]+?)src=[\'"]?([^\'"\s>]+)[\'"]?([^>]*)>/',
			'spa_and_salon_pro_content_image_lazy_load_attr_callback',
			$content
		);
	}
...
add_filter( 'the_content', 'spa_and_salon_pro_content_image_lazy_load_attr' );

if( ! function_exists( 'spa_and_salon_pro_content_image_lazy_load_attr_callback' ) ) :
/**
 ...
 * @return string
 */
function spa_and_salon_pro_content_image_lazy_load_attr_callback( $matches ) {
...
if ( ! function_exists( 'spa_and_salon_pro_js_async_attr' ) ) :
/**
 * 
*/
function spa_and_salon_pro_js_async_attr( $tag ){
    ...
    */
	$async_files = apply_filters( 'spa_and_salon_pro_js_async_files', array( 
...
add_filter( 'script_loader_tag', 'spa_and_salon_pro_js_async_attr', 10 );

if ( ! function_exists( 'spa_and_salon_pro_remove_script_version' ) ) :
/**
 * Remove Script/Style version parameter
*/
function spa_and_salon_pro_remove_script_version( $src ){
...
add_filter( 'script_loader_src', 'spa_and_salon_pro_remove_script_version', 15, 1 );
add_filter( 'style_loader_src', 'spa_and_salon_pro_remove_script_version', 15, 1 );
